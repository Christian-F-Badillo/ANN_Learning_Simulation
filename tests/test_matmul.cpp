...
int main() {
  ...
  // A = [[1, 2, 3],
  //      [4, 5, 6]]
  Math::Matrix<int> A({1, 2, 3, 4, 5, 6}, {2, 3});

  // B = [[7, 8],
  //      [9, 1],
  //      [2, 3]]
  Math::Matrix<int> B({7, 8, 9, 1, 2, 3}, {3, 2});
  ...
  Math::Matrix<int> C = Math::Linalg::matmul(A, B);
  ...
  // A * I = A
  // ---------------------------------------------------------
  TEST_CASE("Matmul: Identity Math::Matrix");

  Math::Matrix<float> mat({1.5f, 2.5f, 3.5f, 4.5f}, {2, 2});
  Math::Matrix<float> identity({1.0f, 0.0f, 0.0f, 1.0f}, {2, 2});

  Math::Matrix<float> resId = Math::Linalg::matmul(mat, identity);
  ...
  Math::Matrix<int> badA({1, 2, 3, 4}, {2, 2});
  Math::Matrix<int> badB({1, 2, 3, 4, 5, 6}, {3, 2});

  ASSERT_THROWS(Math::Linalg::matmul(badA, badB), std::invalid_argument);
  ...
  Math::Matrix<int> rowVec({1, 2, 3}, {1, 3});
  Math::Matrix<int> colVec({4, 5, 6}, {3, 1});

  // 1*4 + 2*5 + 3*6 = 4 + 10 + 18 = 32
  Math::Matrix<int> scalarRes = Math::Linalg::matmul(rowVec, colVec);
  std::cout << scalarRes << '\n';
  ASSERT_EQ(scalarRes.shape()[0], 1);
  ASSERT_EQ(scalarRes.shape()[1], 1);
  ASSERT_EQ(scalarRes.data_ptr()[0], 32);

  TEST_CASE("Transpose: Square Math::Matrix (2x2)");

  Math::Matrix<int> square({1, 2, 3, 4}, {2, 2});

  Math::Matrix<int> tSquare = Math::Linalg::transpose(square);
  ...
  // ---------------------------------------------------------
  TEST_CASE("Transpose: Rectangular Math::Matrix (2x3 -> 3x2)");

  // A = [[1, 2, 3],
  //      [4, 5, 6]]
  Math::Matrix<int> rect({1, 2, 3, 4, 5, 6}, {2, 3});

  Math::Matrix<int> tRect = Math::Linalg::transpose(rect);
  ...
  Math::Matrix<int> rowVec1({10, 20, 30, 40}, {1, 4});
  Math::Matrix<int> colVec1 = Math::Linalg::transpose(rowVec1);
  ...
  Math::Matrix<float> original({0.5f, -0.5f, 1.5f, 2.5f, -2.5f, 3.5f}, {2, 3});

  Math::Matrix<float> doubleT =
      Math::Linalg::transpose(Math::Linalg::transpose(original));
  ...
  Math::Matrix<int> matBias({10, 10, 10, 20, 20, 20}, {2, 3});
  std::vector<int> vecBias = {1, 2, 3};

  try {
    Math::Matrix<int> resBias = matBias + vecBias;
  ...
  Math::Matrix<int> matRow({1, 2, 3, 4, 5, 6}, {2, 3});
  std::vector<int> vecRow = {100, 200};

  try {
    Math::Matrix<int> resRow = matRow + vecRow;
  ...
  Math::Matrix<int> matErr({1, 2, 3, 4}, {2, 2});
